...
public class SelectPokemon extends Application{
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        for (int i = 1; i <= 6; i++) {
            ...
            Button confirm = pokebutton();
            setCoordinates(confirm, 200,250+(i*50));
            tf.setOnAction(e-> checkPokedexNum(tf,pokefield));
            confirm.setOnAction(e->checkPokedexNum(tf,pokefield));
            selectPane.getChildren().addAll(tf,confirm);
            team1[i-1] = tf ;
            buttons1[i-1] = confirm;
        }
        ...
        for (int i = 1; i <= 6; i++) {
            ...
            Button confirm = pokebutton();
            setCoordinates(confirm, 725,250+(i*50));
            tf.setOnAction(e-> checkPokedexNum(tf,pokefield));
            confirm.setOnAction(e->checkPokedexNum(tf,pokefield));
            selectPane.getChildren().addAll(tf,confirm);
            team2[i-1] = tf ;
            buttons2[i-1] = confirm;
        }
    ...
    public boolean checkPokedexNum(TextField choice, ImageView pokefield){
        try{
